#!/usr/bin/env python3
"""One-shot importer: issue PDFs from another newsletter platform -> PiecesOfLife.
...
Usage:
    ...
    pip install pymupdf
    python3 tools/newsletter_pdf_import.py extract \
        --pdf-dir ./pdf-export --out _local/import
    # fill in the emails in _local/import/people.json
    python3 tools/newsletter_pdf_import.py load \
        --bundle _local/import \
...
# ---------------------------------------------------------------------------
# Layout constants, measured from the source platform's export.
#
...
# The source platform's fixed sections become one question per issue.
# "Questions" is the only section carrying real per-issue prompts;
# "Photo Wall" is not a question at all — it maps onto the issue's
# photo & video dump.
SECTIONS = {
...
VIDEO_STILL_CAPTION = "Video still ({duration}) — the video itself is not in the export"
...
def paragraphs(lines):
    ...
    The export encodes paragraph breaks purely by ending a line early — the
...
def cmd_load(args) -> int:
    ...
    # An explicit null email means "not a real member" (e.g. an
...
def check_month_collisions(db, group_id: int, issues: list[dict], allow: bool) -> None:
    ...
    only through the database. A four-weekly cadence produces exactly that
    every 13 issues or so.
    """
...
def resolve_users(db, group_id: int, people: dict, counts: dict) -> dict:
    """Map the export's display names onto users + memberships."""
    users = {}
